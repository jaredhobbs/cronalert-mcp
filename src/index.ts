...
server.tool(
  ...
  "Create a new uptime monitor. Supports HTTP status checks, keyword matching, regex matching, content change detection, and content staleness alerts (Pro+). Set type to 'heartbeat' for cron job / background task monitoring, or 'tcp' (Pro+) to check that a host:port accepts connections — TCP checks can target bare IP addresses, which HTTP checks cannot.",
  {
    ...
    checkInterval: z.number().int().min(30).max(604800).optional().default(180).describe("Heartbeat type: expected seconds between pings, from the plan minimum (60 on Pro+) up to 604800 (7 days). Other types: ignored, set automatically from the plan"),
    gracePeriod: z.number().int().min(0).max(604800).optional().describe("Heartbeat type only: extra seconds a ping may be late before the monitor is down (default: the interval, capped at 3600)"),
...
server.tool(
  ...
  "Update an existing monitor's configuration including content checking mode. Only provided fields are changed.",
  {
    ...
    checkInterval: z.number().int().min(30).max(604800).optional().describe("Heartbeat monitors only: expected seconds between pings (plan minimum to 604800). Ignored for other types"),
    gracePeriod: z.number().int().min(0).max(604800).optional().describe("Heartbeat monitors only: extra seconds a ping may be late before the monitor is down"),
